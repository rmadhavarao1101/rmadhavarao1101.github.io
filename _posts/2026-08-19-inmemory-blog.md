...
A quick sanity check ruled out the boring explanations — the workers were running (`inmemory_max_populate_servers = 2` per node), and an unfiltered `GV$IM_SEGMENTS` confirmed nothing else was competing for the pool. It really was just the size: the full table needs more than the 1.28 GB a 2G store exposes.
...
- [x] **When something won't populate, list the *whole* store, not just your segment.** Run `GV$IM_SEGMENTS` with no `WHERE` clause and check `GV$INMEMORY_AREA` for the pool split. A filtered query hides two of the usual culprits — another segment competing for the same pool, and the 1MB-vs-64KB pool ceiling — so you end up blaming the wrong thing.
